...
GearVR::GearVR() { joy.Clear(); lastjoy.Clear(); config = PointerConfig(); }
...
void GearVR::onNotify(BLERemoteCharacteristic* chr, uint8_t* pData, size_t length, bool isNotify) {
  ...
  if (length < 60) {
    if (length >= 2) {
      uint8_t c0 = pData[0], c1 = pData[1]; (void)c1;
      if      (c0 == kOff[0])    { GVLOG("Req: Off\n"); }
      else if (c0 == kSensor[0]) { GVLOG("Req: Sensor\n"); }
      else if (c0 == kFwUp[0])   { GVLOG("Req: FW Update\n"); }
      else if (c0 == kCal[0])    { GVLOG("Req: Calibrate\n"); }
      else if (c0 == kKeep[0])   { GVLOG("Req: KeepAlive\n"); }
      else if (c0 == kUnk[0])    { GVLOG("Req: Unknown\n"); }
      else if (c0 == kLpmEn[0])  { GVLOG("Req: LPM Enable → queue VR\n"); queueCmd(kVr); }
      else if (c0 == kLpmDis[0]) { GVLOG("Req: LPM Disable\n"); }
      else if (c0 == kVr[0])     {
...
void GearVR::onDisconnected() {
  ...
  pending_ = false; pendingCmd_[0]=pendingCmd_[1]=0x00;
  receiving_ = false; mode_ = 0x00;
}
